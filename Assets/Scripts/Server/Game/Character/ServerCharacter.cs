...
using Server.Game.Action;
using Server.Game.Entity;
using Shared.Data;
using Shared.Game.Action;
    ...
    public class ServerCharacter : NetworkBehaviour
    {
        ...
        private void OnActionPlayRequest(ActionRequestData data)
        {
            if (!GameDataSource.Instance.ActionDataByType[data.ActionTypeEnum].IsFriendly)
            {
                ...
                RunningActions.OnGameplayActivity(Action.Action.GameplayActivity.UsingAttackAction);
            }
        ...
        public void ReceiveHP(ServerCharacter inflicter, int HP)
        {
            ...
            if (HP > 0)
            {
                m_ActionPlayer.OnGameplayActivity(Action.Action.GameplayActivity.Healed);
                float healingMod = m_ActionPlayer.GetBuffedValue(Action.Action.BuffableValue.PercentHealingReceived);
                HP = (int)(HP * healingMod);
            }
            else
            {
                m_ActionPlayer.OnGameplayActivity(Action.Action.GameplayActivity.AttackedByEnemy);
                float damageMod = m_ActionPlayer.GetBuffedValue(Action.Action.BuffableValue.PercentDamageReceived);
                HP = (int)(HP * damageMod);
        ...
        {
            m_ActionPlayer.OnGameplayActivity(Action.Action.GameplayActivity.StoppedChargingUp);
        }
